...
// 1'692.000 + 2'998.000 + 1'874800 + 430.000 + +220.000 + 420.000 + 420.000 + 350.000  =  8'000
//------------------------------------------------------------------------------
...
class My_window : public Graph_lib::Window
{
   public:
    ...
    private:
        ...
        In_box operations;
        Token_stream ts;
        Text t, inst_1, inst_2, inst_3, inst_4, inst_5, inst_6, inst_7, inst_8,
             inst_9, inst_10, credits_1, credits_2, credits_3, credits_4, rslts;
...
My_window::My_window(Point xy, int w, int h, const string& title)
    : Window{xy,w,h,title},
      ...
      inst_6{Point{100,180}, ""}, inst_7{Point{100,200}, ""}, 
      inst_8{Point{100,220}, ""}, inst_9{Point{100,240},""},
      inst_10{Point{100,280}, ""},
      ...
      operations{Point{x_max()-1100,290},1000,40,"Operations: "}
{
    ...
    inst_1.set_label("1. Welcome to our simple calculator, enter the expresion followed by a semicolon (e.g., 3+5;)");
    attach(inst_1);
    inst_2.set_font(Font::times_bold);
    inst_2.set_label("2. Be sure to type the command 'quit;' after providing the instructions.");
    attach(inst_2);
    inst_3.set_font(Font::times);
    inst_3.set_label("3. Special ops: factorial 'n!;', 'pow(n1,n2);', and 'sqrt(n);'.");
    attach(inst_3);
    ...
    inst_5.set_label("5. Load an instructions list from the file 'mydata_calculator' (The file should be attached in the calculator's containing folder). Execute it by 'from mydata_calculator;' command.");
    attach(inst_5);
    inst_6.set_font(Font::times);
    inst_6.set_label("6. Save a list of results in the file 'mydata_calculator_to' (The file should be attached in the calculator's containing folder). Execute it by 'to mydata_calculator_to;' command.");
    attach(inst_6);
    inst_7.set_font(Font::times);
    inst_7.set_label("7. To run a list of commands from a file and save the results to another file: 'to mydata_calculator_to; from mydata_calculator; quit; quit;'. The double 'quit;' is required when using 'to mydata_calculator_to'.");
    attach(inst_7);
    inst_8.set_font(Font::times);
    inst_8.set_label("8. Type 'integers;' to operate on ints (only).");
    attach(inst_8);
    ...
    inst_10.set_label("Enter the expression, for example, 4*6; # var1 = 6; const pi = 3.1416; 5! + var1 - (2*pi); quit;");
    attach(inst_10);
    ...
    credits_1.set_label("The basic code for this calculator is given in PROGRAMMING Principles and Practice Using C++, BJARNE STROUSSTRUP (The creator of C++), second edition.");
    attach(credits_1);
    credits_2.set_font(Font::times_bold);
    credits_2.set_label("The code uses parsing, recursion, and OOP. As part of my language learning process, I introduced some improvements to the calculator -the improvements were");
    attach(credits_2);
    credits_3.set_font(Font::times_bold);
    credits_3.set_label("proposed in the book-. The introduced improvements were: factorial, pow(), sqrt(), use of variables and constants, restriction to the use of integers, help, the connec-");
    attach(credits_3);
    credits_4.set_font(Font::times_bold);
    credits_4.set_label("tion with files ('from' and 'to' commands), the graphical user interface, among others.                                           Sincerely, Camilo Alejandro Castillo Benavides (2022).");
    attach(credits_4);
}
// 
//------------------------------------------------------------------------------
...
int main()
{
    ...
    try
    {
        My_window win{Point{100,100},1350,800,"Calculator"};
        return gui_main();
    }
    catch (exception& e)
    {
