/*6. Make an "analog clock", that is, a clock with hands that move.
In this version of the watch press the "Start" button to update the position
of the hands on the screen.*/
...
class My_window : public Graph_lib::Window
{
	...
	private:
		...
		Button quit_button;
        Button start;
        Text hr_txt;          // Time in text format.
		   ...
		   [](Address,Address pw){reference_to<My_window>(pw).clk();}}
{
    ...
    hr_txt.set_font(Font::times_bold);
    hr_txt.set_font_size(40);
    attach(hr_txt);
...
void My_window::clk()
{
    ...
    hr_txt.set_label(to_string(sc.hour()) + ":" + to_string(sc.min())
                     + ":" + to_string(sc.sec()));
