...
double primary(Token_stream& ts)
{
    ...
    case pwr:                                                         // Cases 'p' (pwr) and ',' handle "pow"'(' expression ')'.
        return pow(primary(ts), narrow_cast<int>(primary(ts)));      // narrow_cast<int> guarantees an integer exponent.
...
double declaration(char ty, Token_stream& ts)                     // ty is 'C' (constant) or 'L' (variable).
{
    Token t = ts.get();
    if (t.kind != name)
