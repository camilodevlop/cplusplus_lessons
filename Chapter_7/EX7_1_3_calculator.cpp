...
The exercises are 1, 2, and 3 of the chapter 7.*/
...
double declaration(char ty)                     // ty is 'C' (constant) or 'L' (variable).
{
    ...
    if (ty == let)
        var_table.push_back(Variable(var_name, d, let));
    else
        var_table.push_back(Variable(var_name, d, cons));
